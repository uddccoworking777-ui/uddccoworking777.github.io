...
(function () {
  ...
  // Maps page filename → gallery category filter (null = no filter)
  const PAGE_CAT = {
  ...
  function galleryItemHTML(item) {
    return `<div class="gallery-item"
      data-cat="${esc(item.category)}" data-category="${esc(item.category)}"
      data-full="${esc(item.src)}"
      data-title="${esc(item.title)}"
      data-meta="${esc(item.meta || '')}">
      <img src="${esc(item.src)}" alt="${esc(item.title)}" loading="lazy" />
      <div class="gallery-item-overlay">
        <div class="gallery-item-cat">${CAT_LABELS[item.category] || esc(item.category)}</div>
  ...
  // Apply after DOM ready (scripts at bottom of body → DOM already parsed)
